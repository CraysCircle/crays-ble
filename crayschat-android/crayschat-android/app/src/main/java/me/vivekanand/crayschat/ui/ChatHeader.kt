...
import me.vivekanand.crayschat.core.ui.utils.singleOrTripleClickable
...
fun PeerCounter(
    ...
    Row(
        verticalAlignment = Alignment.CenterVertically,
        modifier = modifier.clickable { onClick() }.padding(end = 8.dp) // Added right margin to match "Crays Circle" logo spacing
    ) {
...
fun PeerCounterPreview() {
    CraysCircleTheme {
        PeerCounter(
            connectedPeers = listOf("peer1", "peer2", "peer3"),
            joinedChannels = setOf("general", "random"),
            hasUnreadChannels = mapOf("general" to 2, "random" to 1),
...
@Preview(showBackground = true, backgroundColor = 0xFFFFFFFF)
@Composable
fun PrivateChatHeaderPreview() {
    CraysCircleTheme {
        PrivateChatHeader(
            peerID = "peer123",
            peerNicknames = mapOf("peer123" to "Alice"),
            isFavorite = true,

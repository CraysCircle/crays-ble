...
/**
 * Centralized permission management for Crays Circle app
...
class PermissionManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val PREFS_NAME = "crayschat_permissions"
        ...
        if (isBatteryOptimizationSupported()) {
            categories.add(
                PermissionCategory(
                    ...
                    description = "Disable battery optimization to ensure Crays Circle runs reliably in the background and maintains mesh network connections",
                    permissions = listOf("BATTERY_OPTIMIZATION"), // Custom identifier
                    isGranted = isBatteryOptimizationDisabled(),
                    systemDescription = "Allow Crays Circle to run without battery restrictions"
                )
            )
        }

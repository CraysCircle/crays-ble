...
class MessageHandler(private val myPeerID: String) {
    ...
    suspend fun handleNoiseEncrypted(routed: RoutedPacket) {
        ...
        try {
            ...
            val innerPacket = CraysChatPacket.fromBinaryData(decryptedData)
            if (innerPacket != null) {
    ...
    private suspend fun handleBroadcastMessage(routed: RoutedPacket) {
        ...
        try {
            // Parse message
            val message = CraysChatMessage.fromBinaryPayload(packet.payload)
            if (message != null) {
     ...
     */
    private suspend fun handlePrivateMessage(packet: CraysChatPacket, peerID: String) {
        try {
...
interface MessageHandlerDelegate {
    ...
    // Packet operations
    fun sendPacket(packet: CraysChatPacket)
    fun relayPacket(routed: RoutedPacket)
    fun getBroadcastRecipient(): ByteArray
    
    // Cryptographic operations
    fun verifySignature(packet: CraysChatPacket, peerID: String): Boolean
    ...
    // Callbacks
    fun onMessageReceived(message: CraysChatMessage)

package me.vivekanand.crayschat.services
...
class MessageRetentionService private constructor(private val context: Context) {
    ...
    suspend fun saveMessage(message: CraysChatMessage, forChannel: String) = withContext(Dispatchers.IO) {
    ...
    suspend fun loadMessagesForChannel(channel: String): List<CraysChatMessage> = withContext(Dispatchers.IO) {
    ...
    private fun loadMessagesFromFile(file: File): List<CraysChatMessage> {
        if (!file.exists()) {
            return emptyList()
        }
        ...
        return try {
            FileInputStream(file).use { fis ->
                ObjectInputStream(fis).use { ois ->
                    ...
                    ois.readObject() as List<CraysChatMessage>
                }
            }
        } catch (e: Exception) {
    ...
    private fun saveMessagesToFile(file: File, messages: List<CraysChatMessage>) {

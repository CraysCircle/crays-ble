...
/**
 * Refactored ChatViewModel - Main coordinator for crayschat functionality
...
class ChatViewModel(
    ...
    val messages: LiveData<List<CraysChatMessage>> = state.messages
    ...
    val privateChats: LiveData<Map<String, List<CraysChatMessage>>> = state.privateChats
    ...
    val channelMessages: LiveData<Map<String, List<CraysChatMessage>>> = state.channelMessages
    ...
    private fun loadAndInitialize() {
        ...
        viewModelScope.launch {
            ...
            if (state.getConnectedPeersValue().isEmpty() && state.getMessagesValue().isEmpty()) {
                val welcomeMessage = CraysChatMessage(
                    sender = "system",
                    content = "get people around you to download crayschat and chat with them here!",
                    timestamp = Date(),
                    isRelay = false
                )
    ...
    private fun clearAllCryptographicData() {
        try {
            ...
            try {
                val identityManager = me.vivekanand.crayschat.identity.SecureIdentityStateManager(getApplication())
